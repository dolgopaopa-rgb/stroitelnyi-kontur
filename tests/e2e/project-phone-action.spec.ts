...
test("project card shows the customer phone and opens clear contact actions", async ({ page, context }) => {
  ...
  try {
    ...
    const callLink = page.getByTestId("phone-call-link");
    await expect(callLink).toHaveText("Позвонить");
    await expect(callLink).toHaveAttribute("href", "tel:+79091234567");
    const callLinkColors = await callLink.evaluate((node) => {
      const styles = getComputedStyle(node);
      return { color: styles.color, backgroundColor: styles.backgroundColor };
    });
    expect(callLinkColors.color).not.toBe(callLinkColors.backgroundColor);
