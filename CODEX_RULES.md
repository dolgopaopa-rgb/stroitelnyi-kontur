...
13. Раздел "Сметы" проверять настоящим touch-свайпом на 390x844 до и после открытия и закрытия формы. Подмена свайпа через `window.scrollTo()` не считается проверкой touch-scroll.
14. Создание задания на смету проверять двойным `requestSubmit()`: сервер должен получить ровно один запрос создания.
15. Прикрепление 12 и более файлов к новому заданию проверять реальными синтетическими файлами: задание создаётся один раз, файлы отправляются частями, пользователь видит ход загрузки.
16. Если обязательный мобильный сценарий не нашёл ни одной рабочей карточки, тест должен завершиться ошибкой, а не `OK` или `skip`.
